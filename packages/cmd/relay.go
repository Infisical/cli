...
var relayStartCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if instanceType == "instance" {
			relayAuthSecret := os.Getenv(connector.RELAY_AUTH_SECRET_ENV_NAME)
			if relayAuthSecret == "" {
				util.HandleError(fmt.Errorf("%s is not set", connector.RELAY_AUTH_SECRET_ENV_NAME), "unable to get relay auth secret")
			}
